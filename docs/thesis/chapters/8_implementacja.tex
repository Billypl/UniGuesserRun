...
Do implementacji funkcjonalności związanej z lokalizacją użytkownika wykorzystano wbudowane w przeglądarki Geolocation API. Mechanizm ten działa tylko w kontekście bezpiecznym (HTTPS / localhost) i wymaga jawnej zgody użytkownika. W aplikacji wykorzystano zarówno jednorazowe pobieranie pozycji (getCurrentPosition), jak i ciągłe śledzenie (watchPosition) dla trybów gry, które wymagają aktualizacji współrzędnych w czasie rzeczywistym.
...
Poniżej znajduje się przykładowy fragment kodu (TypeScript) użyty we frontendzie, ilustrujący sposób integracji z Geolocation API:

\begin{lstlisting}[language=JavaScript, caption={Przykład wykorzystania Geolocation API do śledzenia pozycji użytkownika}]
...
\begin{lstlisting}[language=JavaScript, caption={Przykład użycia Geolocation API we własnym hooku}]
...
Poniżej znajduje się przykładowy fragment kodu (TypeScript) przedstawiający implementację serwisu do zarządzania miejscami (PlaceService) z wykorzystaniem tokenu JWT do autoryzacji:

\begin{lstlisting}[language=JavaScript, caption={Konfiguracja instancji Axios z tokenem JWT}]
export class PlaceService {
    ...
    // Metoda pobierająca wszystkie miejsca w kolejce do sprawdzenia, dostępna tylko dla autoryzowanych użytkowników
...
Poniżej znajduje się użyta konfiguracja Vite, która zawiera zarówno rejestrację Service Workera, jak i definicję manifestu aplikacji:

\begin{lstlisting}[language=JavaScript, caption={Konfiguracja Vite do obsługi PWA}]
...
Główny komponent \texttt{Game} odpowiada za przebieg sesji gry. Po załadowaniu sprawdzana jest możliwość wznowienia gry poprzez istniejący token lub inicjalizowana jest nowa rozgrywka.

\begin{lstlisting}[language=JavaScript, caption={Fragment kodu w komponencie \texttt{Game} odpowiedzialny za decyzję o wznowieniu lub rozpoczęciu nowej gry}]
useEffect(() => {
...
Następnie, jeśli rozgrywka z poprzedniej sesji ma być kontynuowana, wywoływana jest odpowiednia runda gry, na podstawie danych uzyskanych z serwera.

\begin{lstlisting}[language=JavaScript, caption={Funkcja pobierająca stan gry w komponencie \texttt{Game}}]
...
W przeciwnym wypadku, tworzona jest nowa sesja na podstawie ustawień zapisanych w \texttt{sessionStorage} przez \texttt{GameSettings}.

\begin{lstlisting}[language=JavaScript, caption={Funkcja rozpoczynająca nową grę w komponencie \texttt{Game}}]
