...
Przy realizacji projektu \textit{UniGuesser} wykorzystano szereg narzędzi wspomagających zarządzanie kodem źródłowym, konteneryzację aplikacji, zarządzanie projektem, projektowanie interfejsu użytkownika oraz środowiska programistyczne. Poniżej przedstawiono omówienie najważniejszych z nich.
...
W projekcie \textit{UniGuesser} GitHub pełnił rolę centralnego punktu współpracy, umożliwiając między innymi zgłaszanie błędów, przeglądanie zmian oraz automatyczne uruchamianie testów jednostkowych i wdrożeń (CI/CD).
...
\section{Konteneryzacja (Michał Pawiłojć)}
...
Użycie Dockera i Composa przyniosło wiele korzyści, między innymi:
...
\section{Zarządzanie projektem (Karina Wołoszyn)}
...
\section{Zintegrowane środowisko programistyczne (Michał Pawiłojć)}
...
Visual Studio oferowało pełny zestaw funkcjonalności ułatwiających pracę z aplikacjami ASP.NET Core, między innymi:
\begin{itemize}
...
\begin{itemize}
    \item \textbf{Standaryzacja formatowania kodu} --- z wykorzystaniem plików konfiguracyjnych \linebreak (\texttt{.editorconfig}, \texttt{.prettierrc}),
...
\section{Projektowanie interfejsu użytkownika (Karina Wołoszyn)}
