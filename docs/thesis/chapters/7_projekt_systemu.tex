...
są obecnie standardem w wytwarzaniu oprogramowania.
...
\begin{itemize}
  ...
  \item Jedno źródło danych zapewnia uproszczenie architektury i zmniejszenie kosztów utrzymania systemu.
...
Model systemu zaprojektowano zgodnie z podejściem \textbf{Domain-Driven Design} (DDD), które koncentruje się na odwzorowaniu rzeczywistych reguł biznesowych w postaci niezależnego modelu domenowego. W połączeniu z zasadami \textbf{Czystej Architektury} (Clean Architecture) uzyskano wyraźny kierunek zależności: warstwy zewnętrzne zależą od wewnętrznych, natomiast rdzeń domeny pozostaje izolowany od szczegółów technicznych. W rezultacie aplikacja została podzielona na cztery główne warstwy:
\begin{itemize}
  \item \textbf{Warstwa API} - punkt wejściowy systemu, który odbiera żądania użytkowników (HTTP), mapuje je na komendy/zapytania i przekazuje do warstwy Application.
  \item \textbf{Aplikacji} - warstwa zarządzająca przypadkami użycia: waliduje żądania, koordynuje operacje i wywołuje logikę domenową.
  \item \textbf{Domeny} - serce aplikacji, zawierająca modele domenowe, encje, agregaty oraz logikę biznesową.
  \item \textbf{Infrastruktury} - warstwa techniczna odpowiedzialna za zapis i odczyt danych (baza, pliki, integracje), implementująca repozytoria i usługi potrzebne aplikacji.
\end{itemize}
\smallskip

Warstwa prezentacji (frontend) komunikuje się wyłącznie z warstwą API. Warstwa API deleguje logikę do warstwy Application, ta korzysta z modelu domenowego (Domain), który z kolei używa Infrastructure do dostępu do bazy danych PostgreSQL.
...
W niniejszej sekcji przedstawiono kluczowe elementy modelu danych zastosowanego w projekcie. Omówiono najważniejsze tabele, ich strukturę oraz wzajemne powiązania, które umożliwiają poprawne odwzorowanie logiki aplikacji. Zaprezentowane zależności ilustrują, w jaki sposób dane dotyczące użytkowników, rozgrywek oraz lokalizacji łączą się w spójną całość, wspierając zarówno działanie systemu, jak i późniejszą analizę informacji.

\begin{table}[H]
  \caption{Tabela \texttt{Users} przechowująca informacje o użytkownikach aplikacji.}
  \centering
...
\begin{table}[H]
  \centering
  \caption{Tabela \texttt{Places} przechowująca informacje o lokacjach dostępnych w grze.}
  \renewcommand{\arraystretch}{1.2}
...
\begin{table}[H]
  \centering
  \caption{Tabela \texttt{GameSessions} przechowująca informacje o aktywnych i zakończonych rozgrywkach.}
  \renewcommand{\arraystretch}{1.2}
...
\begin{table}[H]
  \centering
  \caption{Tabela \texttt{Rounds} przechowująca informacje o poszczególnych rundach w sesji gry.}
  \renewcommand{\arraystretch}{1.2}
...
\begin{figure}[H]
  \centering
  \makebox[\textwidth]{\includegraphics[width=0.4\textwidth]{./images/registerForm.png}}
