        ...
        HasGameSessionInDatabase requirement)
    {
            ...
            !Guid.TryParse(gameGuidString, out var gameGuid))
        {
            if (!routeValues.TryGetValue("gameGuid", out var guidObj2))
                throw new Exception(
                    $"-1: routeValues.TryGetValue {routeValues.TryGetValue("gameGuid", out var guidObj3)}");

            if (guidObj2 is not string gameGuidString2) throw new Exception($"2: guidObj not string {guidObj2}");

            if (!Guid.TryParse(gameGuidString2, out var gameGuid2))
                throw new Exception(
                    $"3: gameGuidString2 {gameGuidString2}, Guid.TryParse {!Guid.TryParse(gameGuidString2, out var gameGuid4)}");
        ...
        var isAuthorized = tokenType switch
        {
