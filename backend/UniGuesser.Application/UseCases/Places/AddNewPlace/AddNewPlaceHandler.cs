...
public class AddNewPlaceHandle(
    IPlacesRepository placesRepository,
    IMapper mapper,
    IAccountRepository accountRepository,
    IHttpContextAccessorService httpContextAccessorService,
    IFileService fileService) : IRequestHandler<AddNewPlaceCommand, Unit>
{
    public async Task<Unit> Handle(AddNewPlaceCommand newPlace, CancellationToken cancellationToken)
    {
        var authorData = httpContextAccessorService.GetAuthenticatedUserProfile();

        // Ensure null safety by checking the result of GetByPublicIdAsync
        var user = await accountRepository.GetAsync(Guid.Parse(authorData.Guid));
        if (user == null) throw new InvalidOperationException($"User with Id {authorData.Guid} not found.");

        var newPlaceToCheck = mapper.Map<Place>(newPlace.NewPlaceDto);
        ...
        if (newPlace.FormFile is null && newPlace.NewPlaceDto.ImageType == ImageType.File)
            throw new ArgumentNullException(nameof(newPlace.FormFile),
                "FormFile cannot be null when ImageType is File.");
