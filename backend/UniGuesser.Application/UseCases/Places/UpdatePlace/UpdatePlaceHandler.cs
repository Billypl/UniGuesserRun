...
public class UpdatePlaceHandler(IPlacesRepository placesRepository, IMapper mapper)
    : IRequestHandler<UpdatePlaceCommand, Unit>
{
    public async Task<Unit> Handle(UpdatePlaceCommand request, CancellationToken cancellationToken)
    {
        var place = await placesRepository.GetAsync(request.PlaceId);

        if (place is null) throw new PlacesExceptions.PlaceNotFoundException(request.PlaceId);
